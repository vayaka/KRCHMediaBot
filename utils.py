...
async def send_request_to_model_swap(target_image, swap_image, user_id):
    result = await predictor.predict(target_image, swap_image)
    if result:
        return result
    return None

async def send_request_to_model_gan(result, target_image, user_id):
    output_path = await predictor.enhance_result(result, user_id, target_image)
    if output_path:
        return output_path
    return ""
...
async def generate_model(target_images_path, swap_image_path, user_id):
    """
    Обрабатывает папку с целевыми изображениями и одно свап-изображение,
    выполняет асинхронные запросы для создания новых изображений.
    ...
    # Асинхронно обрабатываем каждое целевое изображение
    async def process_target_image_swap(target_image_path):

        # Отправляем запрос на API
        print(target_image_path, " send to model swap")
        result_output = await send_request_to_model_swap(target_image_path, swap_image_path, user_id)
    ...
    async def process_target_image_gan(result, target_image_path, user_id):

        # Отправляем запрос на API
        print(target_image_path, " send to model swap")
        result_output = await send_request_to_model_gan(result, target_image_path, user_id)
    ...
    # Запускаем обработку изображений параллельно
    results = await asyncio.gather(*[process_target_image_swap(path) for path in target_images])
    outputs = await asyncio.gather(*[process_target_image_gan(res, path, user_id) for res, path in zip(results, target_images)])
